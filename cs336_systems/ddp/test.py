...
FIXED_BATCH_PATH = "fixed_batch.pt"


def prepare_fixed_batch(cfg, seed=123):
    """Generate ONE fixed global batch on CPU and save to disk.
    Only data changes; everything else stays the same.
    """
    if os.path.exists(FIXED_BATCH_PATH):
        return
    ...
    torch.save({"x": x, "y": y}, FIXED_BATCH_PATH)
...
# -------------------------
# ONLY CHANGED PART: DATA
# -------------------------

def generate_data_batch(cfg, device, generator):
    data = torch.load(FIXED_BATCH_PATH, map_location="cpu")
...
def generate_and_scatter_data(rank, world_size, cfg, device, generator):
    ...
    if rank == 0:
        data = torch.load(FIXED_BATCH_PATH, map_location="cpu")
        global_x = data["x"].to(device, non_blocking=True)
        global_y = data["y"].to(device, non_blocking=True)
...
def main():
    ...
    # NEW: generate ONE fixed batch once (4 samples)
    prepare_fixed_batch(cfg, seed=seed)
    ...
    # optional: clean fixed batch too
    if os.path.exists(FIXED_BATCH_PATH):
        os.remove(FIXED_BATCH_PATH)
