...
def run_single(backend, cfg, use_flash, warmup, nsteps, seed=123):
    ...
    for step in range(nsteps):
        ...
        if step == 0:
            fingerprint_batch(x, y, f"SINGLE(use_flash={use_flash})")
...
def fingerprint_batch(x: torch.Tensor, y: torch.Tensor, tag: str):
    xs = int(x.sum().item())
    x0 = int(x.reshape(-1)[0].item())
    xlast = int(x.reshape(-1)[-1].item())

    ys = int(y.sum().item())
    y0 = int(y.reshape(-1)[0].item())
    ylast = int(y.reshape(-1)[-1].item())

    print(
        f"[{tag}] "
        f"x(sum={xs}, first={x0}, last={xlast}) | "
        f"y(sum={ys}, first={y0}, last={ylast})"
    )
...
def generate_and_scatter_data(rank, world_size, cfg, device, generator):
    ...
    fingerprint_batch(x, y, "DDP_RANK")
